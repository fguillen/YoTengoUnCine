...
class Admin::AdminHelperTest < ActionView::TestCase
  def test_admin_menu_class
    ...
    assert_equal("active", admin_menu_class(:donations))

    request_mock.stubs(:fullpath).returns("/admin/donations")
    assert_equal("active", admin_menu_class(:donations))

    request_mock.stubs(:fullpath).returns("/admin/donations/1")
    assert_equal("active", admin_menu_class(:donations))

    request_mock.stubs(:fullpath).returns("/admin/donations/1/edit")
    assert_equal("active", admin_menu_class(:donations))
